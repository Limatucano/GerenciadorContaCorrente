...
interface ActionsDAO {
    ...
    @Query("SELECT sum(value) as value FROM Actions WHERE contaCorrente = :contaCorrente AND (actionType = 'saque' OR actionType = 'visita' OR actionType = 'tarifa' OR (actionType = 'transferencia' AND actionTo != :contaCorrente))")
    ...
    @Query("SELECT date, description, value FROM Actions WHERE contaCorrente = :contaCorrente or actionTo = :contaCorrente")
